    ...
    function generate_custom_field_validation($custom_fields) {
        ...
        if (!empty($custom_fields)) {
            foreach ($custom_fields as $field) {
                $validationRules['custom_fields.' . $field->field_name] = $field->validation;

                if ($field->field_type == 'file') {
                    $max_size                                               = $field->max_size * 1024;
                    $validationRules['custom_fields.' . $field->field_name] = $field->validation . "|mimes:jpeg,jpg,png,pdf|max:$max_size";
                }
